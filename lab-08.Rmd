...
linRain <- lm(RainTomorrow ~ RainToday, data = weather_noNA)
tidy(linRain)
...
```{r}
RainToday <- as.logical(as.character(RainToday))
RainTomorrow <- as.logical(as.character(RainTomorrow))

ggplot(data=linRain, aes(x = RainToday, y = RainTomorrow)) +
  geom_point() +
